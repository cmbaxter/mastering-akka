...
object SalesOrderProcessor{
  ...
  case class UnresolvedDependencies(inputs:Inputs, userMgr:Option[ActorRef] = None, bookMgr:Option[ActorRef] = None, creditHandler:Option[ActorRef] = None) extends InputsData
  case class ResolvedDependencies(inputs:Inputs, expectedBooks:Set[Int], user:Option[BookstoreUser], books:Map[Int, Book], userMgr:ActorRef, bookMgr:ActorRef, creditHandler:ActorRef) extends InputsData
  case class LookedUpData(inputs:Inputs, user:BookstoreUser, items:List[SalesOrderLineItem], total:Double) extends InputsData
...
class SalesOrderProcessor extends FSM[SalesOrderProcessor.State, SalesOrderProcessor.Data]{
  ...
  when(LookingUpEntities, 10 seconds)(transform {
    case Event(FullResult(b:Book), data:ResolvedDependencies) =>      
      ...
      lineItemForBook match{
        case None =>
          log.error("Got back a bok for which we don't have a line item")
  ...
  when(ChargingCard, 10 seconds){
    case Event(FullResult(txn:CreditCardTransaction), data:LookedUpData) if txn.status == CreditTransactionStatus.Approved  =>
      ...
      val order = SalesOrder(0, data.user.id, txn.id, SalesOrderStatus.InProgress, data.total, data.items, new Date, new Date)
