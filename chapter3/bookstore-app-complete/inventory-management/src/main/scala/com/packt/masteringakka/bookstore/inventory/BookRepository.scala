...
object BookRepository{
  implicit val GetBook = GetResult{r => BookVO(r.<<, r.<<, r.<<, r.nextString.split(",").filter(_.nonEmpty).toList, r.<<, r.<<, r.nextTimestamp, r.nextTimestamp)} 
}
...
class BookRepository(implicit ec:ExecutionContext) extends BookstoreRepository{
  ...
  def loadBook(id:Int) = {
    ...
    """
    db.run(query.as[BookVO].map(_.headOption))
  }
